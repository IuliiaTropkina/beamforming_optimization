...
from tf_agents.trajectories.time_step import restart, transition, termination
...
class GridWorldEnv(py_environment.PyEnvironment):
    ...
    def _reset(self):
        self._state=[0,0,5,5]
        self._episode_ended = False
        return restart(np.array(self._state, dtype=np.int32))
    ...
    def _step(self, action):
        ...
        if self._episode_ended:
            if self.game_over():
                ...
                reward = 0
            return termination(np.array(self._state, dtype=np.int32), reward)
        else:
            return transition(
